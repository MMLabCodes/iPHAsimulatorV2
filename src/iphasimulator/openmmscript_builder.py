...
class OpenMMScriptBuilder:
    ...
    def add_minimization(self):
        self.steps.append({"method": "minimize_energy"})
...
'''
    ...
    def _build_steps_code(self):
        if len(self.steps) == 0:
            return textwrap.indent('print("No simulation steps were added.")', "    ")
        ...
        for index, step in enumerate(self.steps, start=1):
            ...
            if method == "minimize_energy":
                ...
                if method == "basic_NVT":
                    lines.extend(self._format_basic_NVT(step))

                elif method == "basic_NPT":
                    lines.extend(self._format_basic_NPT(step))

                elif method == "anneal_NVT":
                    lines.extend(self._format_anneal_NVT(step))

                elif method == "thermal_ramp":
                    lines.extend(self._format_thermal_ramp(step))

                else:
                    raise ValueError(f"Unknown OpenMM workflow method: {method}")
